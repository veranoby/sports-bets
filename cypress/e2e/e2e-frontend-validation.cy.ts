...
describe('End-to-End Frontend Validation of PRD Business Logic', () => {
  before(() => {
  ...
  describe('Layer 1: Navigation Routing Validation', () => {
    it('validates user navigation to /eventos', () => {
      cy.visit('/events');
    ...
    it('validates clicking event card navigates to /eventos/:id', () => {
      cy.visit('/events');
    ...
    it('validates admin navigation to /admin/eventos', () => {
      // Mock admin user
      cy.intercept('GET', '/api/users/profile', {
        statusCode: 200,
        body: {
  ...
  describe('Layer 2: Component Interactions Validation', () => {
    beforeEach(() => {
    ...
    it('validates BettingPanel visibility when fight status = \'betting\'', () => {
      // Mock SSE to send betting window opened event
      cy.intercept('GET', '/api/sse/events*', (req) => {
        req.reply({
          statusCode: 200,
          headers: {
    ...
    it('validates BettingPanel disabled when fight status = \'upcoming\'', () => {
    ...
    it('validates countdown timer updates during betting window', () => {
      cy.visit('/events/event-123');
  ...
  describe('Layer 3: Form Submissions Validation', () => {
    beforeEach(() => {
    ...
    it('validates user can submit bet on red rooster', () => {
      cy.visit('/events/event-123');
      ...
      cy.wait('@createBet').then((interception) => {
    ...
    it('validates admin can create new event', () => {
      // Mock admin user
      cy.intercept('GET', '/api/users/profile', {
        statusCode: 200,
        body: {
  ...
  describe('Layer 4: Realtime Updates Validation', () => {
    it('validates SSE connection established on LiveEvent page load', () => {
      cy.visit('/events/event-123');
    ...
    it('validates fight status change broadcasted via SSE', () => {
      cy.visit('/events/event-123');
      
      // Mock SSE status change
      const statusChangeEvent = new Event('BETTING_WINDOW_CLOSED');
      cy.window().then((win) => {
    ...
    it('validates PAGO bet proposal via WebSocket', () => {
  ...
  describe('Layer 5: Business Logic Compliance Validation', () => {
    ...
    it('validates temporal betting window enforcement (PRD 87-96)', () => {
      cy.visit('/events/event-123');
    ...
    it('validates role-based Navigation menu restrictions (gemini-prompt TASK_B)', () => {
    ...
    it('validates subscription gate for premium content (PRD 149-153)', () => {
  ...
  describe('Additional PRD Compliance Tests', () => {
    it('validates fight temporal logic from PRD (betting only during betting status)', () => {
