...
const setupBettingSocket = (io) => {
    ...
    io.of('/betting').use((socket, next) => __awaiter(void 0, void 0, void 0, function* () {
        try {
    ...
    io.of('/betting').on('connection', (socket) => {
        const { userId, fightId, role } = socket.data;
        logger_1.logger.info(`Betting user connected: ${userId} for fight ${fightId || 'none'}`);
        ...
        socket.on('create_pago_bet', (data) => __awaiter(void 0, void 0, void 0, function* () {
            try {
                ...
                pendingBet.timeout = setTimeout(() => {
                    ...
                    logger_1.logger.info(`PAGO bet ${betId} expired due to timeout`);
                }, timeout);
                ...
                logger_1.logger.info(`PAGO bet ${betId} created by user ${userId}`);
            }
            catch (error) {
                logger_1.logger.error('Error creating PAGO bet:', error);
                socket.emit('bet_error', { message: 'Failed to create PAGO bet' });
            }
        }));
        // Handle DOY bet response (accepting a PAGO bet)
        socket.on('accept_pago_bet', (data) => __awaiter(void 0, void 0, void 0, function* () {
            try {
                ...
                logger_1.logger.info(`PAGO bet ${data.betId} accepted by user ${userId}`);
            }
            catch (error) {
                logger_1.logger.error('Error accepting PAGO bet:', error);
        ...
        socket.on('cancel_bet', (data) => {
            ...
            if (pendingBet && pendingBet.userId === userId) {
                ...
                logger_1.logger.info(`Bet ${data.betId} cancelled by user ${userId}`);
            }
            else {
        ...
        socket.on('join_fight_betting', (data) => {
        ...
        socket.on('disconnect', (reason) => {
            logger_1.logger.info(`Betting user disconnected: ${userId} - ${reason}`);
            const bettor = activeBettors.get(socket.id);
            if (bettor) {
    ...
    setInterval(() => {
        ...
        for (const [betId, bet] of pendingBets.entries()) {
            ...
            if (elapsed > 120000) {
                ...
                pendingBets.delete(betId);
                logger_1.logger.info(`Auto-expired old pending bet: ${betId}`);
            }
        }
...
const forceCancelBet = (betId, reason = 'admin_action') => {
