...
router.post('/',
  ...
  asyncHandler(async (req, res) => {
    ...
    if (existingFight) {
      throw errors.conflict('A fight with this number already exists in the event');
    }
...
router.put('/:id',
  ...
  asyncHandler(async (req, res) => {
    ...
    // Verificar permisos del operador
    if (req.user!.role === 'operator' && fight.event.operatorId !== req.user!.id) {
      throw errors.forbidden('You are not assigned to this event');
    }
    ...
    const allowedFields = ['redCorner', 'blueCorner', 'weight', 'notes', 'status'];
      ...
      for (const bet of fight.bets) {
        ...
        if (wallet) {
          ...
          await require('../models/Wallet').Transaction.create({
            ...
            type: betResult === 'win' ? 'bet-win' : 
                  betResult === 'cancelled' || betResult === 'draw' ? 'bet-refund' : 'bet-loss',
            amount: betResult === 'win' ? bet.potentialWin : 
                   betResult === 'cancelled' || betResult === 'draw' ? bet.amount : bet.amount,
            status: 'completed',
            description: `${betResult === 'win' ? 'Won' : 
                         betResult === 'cancelled' || betResult === 'draw' ? 'Refund for' : 'Lost'} bet on fight ${fight.number}`,
            metadata: {
              fightId: fight.id,
              betId: bet.id,
              result: result
