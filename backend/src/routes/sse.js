...
router.get('/admin/system-status', sseHeaders, (req, res) => {
    const clientId = sseService_1.sseService.addConnection(res);
    console.log(`Client connected to system-status: ${clientId}`);
    // Send a confirmation message
    sseService_1.sseService.sendToClient(clientId, { type: 'SYSTEM_STATUS_CONNECTED', data: 'Connection established' });
    req.on('close', () => {
        sseService_1.sseService.removeConnection(clientId);
...
router.get('/admin/events/:eventId', sseHeaders, (req, res) => {
    ...
    const clientId = sseService_1.sseService.addConnection(res, `event-${eventId}`);
...
router.get('/admin/notifications', sseHeaders, (req, res) => {
    const clientId = sseService_1.sseService.addConnection(res, 'admin-notifications');
...
router.get('/events/:eventId/stream', sseHeaders, (req, res) => {
    const { eventId } = req.params;
    const clientId = sseService_1.sseService.addConnection(res, `stream-${eventId}`);
...
router.get('/fights/:fightId/bets', sseHeaders, (req, res) => {
    ...
    const clientId = sseService_1.sseService.addConnection(res, `fight-${fightId}`);
