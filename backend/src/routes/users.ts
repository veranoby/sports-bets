...
router.put('/:id/status',
  ...
  asyncHandler(async (req, res) => {
    ...
    require('../config/logger').logger.info(
      `User ${user.username} (${user.id}) ${isActive ? 'activated' : 'deactivated'} by admin ${req.user!.username}. Reason: ${reason || 'Not specified'}`
    );
...
router.put('/:id/role',
  ...
  asyncHandler(async (req, res) => {
    ...
    // Log de auditoría
    require('../config/logger').logger.info(
      `User ${user.username} (${user.id}) role changed from ${oldRole} to ${role} by admin ${req.user!.username}. Reason: ${reason || 'Not specified'}`
    );
...
router.get('/operators/available',
  authenticate,
  authorize('admin'),
  asyncHandler(async (req, res) => {
