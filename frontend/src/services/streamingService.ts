...
class StreamingService {
  ...
  async stopStream(
    ...
    ApiResponse<{ success: boolean; duration: number; viewerCount: number }>
  > {
    return apiCall<{ success: boolean; duration: number; viewerCount: number }>(
      "post",
      "/streaming/stop",
      { streamId },
    );
  ...
  async getStreamAccess(
    eventId: string,
  ): Promise<ApiResponse<StreamAccessToken>> {
    return apiCall<StreamAccessToken>(
      "get",
  ...
  async validateStreamToken(
    token: string,
  ): Promise<ApiResponse<{ valid: boolean; userId: number; eventId: string }>> {
    return apiCall<{ valid: boolean; userId: number; eventId: string }>(
      "post",
      "/streaming/validate-token",
      { token },
    );
  }

  async reportViewingEvent(
    eventId: string,
    event: string,
  ...
  async getAvailableQualities(streamId: string): Promise<
    ApiResponse<
      Array<{
        label: string;
        src: string;
  ...
  async toggleRecording(
    ...
    return apiCall<void>("post", `/streaming/${streamId}/recording`, {
