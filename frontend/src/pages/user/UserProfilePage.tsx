...
import { MapPin, ChevronLeft, BookOpen, Calendar, User as UserIcon } from 'lucide-react';
...
const UserProfilePage: React.FC = () => {
  ...
  if (!user) return <EmptyState title="Usuario no encontrado" icon={<UserIcon />} />;
      ...
      <Card className="p-6">
        <div className="flex flex-col sm:flex-row items-start gap-6">
            ...
            <div className="flex-1">
                <h1 className="text-3xl font-bold text-theme-primary mb-2">{profileName}</h1>
        ...
        {articles.length > 0 ? (
            <div className="space-y-4">
                {articles.map(article => (
                    <div key={article.id} className="p-4 rounded-lg bg-gray-800/50 hover:bg-gray-700/50 cursor-pointer">
                        <h3 className="font-semibold text-theme-primary">{article.title}</h3>
                        <p className="text-sm text-theme-secondary line-clamp-2 mt-1">{article.summary}</p>
                        <div className="text-xs text-theme-light mt-2 flex items-center gap-2">
                            <Calendar className="w-3 h-3"/>
                            <span>{new Date(article.createdAt || article.created_at || Date.now()).toLocaleDateString()}</span>
                        </div>
                    </div>
                ))}
            </div>
        ) : (
            <EmptyState title="Sin artículos" description={`${profileName} aún no ha publicado artículos.`} />
        )}
      </Card>
    </div>
  );
