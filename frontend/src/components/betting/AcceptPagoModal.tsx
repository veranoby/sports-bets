  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (action === 'accept') {
        ...
        response = await rejectPago(pagoBet.id, 'Rechazado por el propietario de la apuesta original');
      }
      ...
      console.error(`Error ${action === 'accept' ? 'aceptando' : 'rechazando'} propuesta PAGO:`, err);
      setError(err.message || `Error al ${action === 'accept' ? 'aceptar' : 'rechazar'} la propuesta PAGO`);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="space-y-5">
        ...
        <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-semibold text-blue-900">Propuesta PAGO Recibida</h3>
          </div>
          ...
          <div className="space-y-2 text-sm">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-blue-900">{pagoBet.userId?.substring(0, 8)}...</p>
              </div>
              <div>
                <span className="text-blue-700 font-medium">Monto PAGO:</span>
                <p className="text-blue-900">{formatCurrency(pagoBet.amount)}</p>
              </div>
            </div>
            ...
            <div className="pt-2 border-t border-blue-200">
              ...
              <p className="text-blue-900">
                {pagoBet.userId} quiere pagar ${pagoBet.amount} para cubrir tu apuesta de ${originalBet.amount}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-2 text-sm">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-gray-900">{originalBet.side === 'red' ? '🔴 Rojo' : '🔵 Azul'}</p>
              </div>
              <div>
                <span className="text-gray-700 font-medium">Monto:</span>
                <p className="text-gray-900">{formatCurrency(originalBet.amount)}</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 gap-3">
            ...
            >
              ...
              <div className="text-left">
                ...
                <div className="text-sm">
                  Aceptas que {pagoBet.userId?.substring(0, 8)}... pague ${pagoBet.amount} para cubrir tu apuesta
                </div>
              </div>
            </button>
            ...
            >
              ...
              <div className="text-left">
                ...
                <div className="text-sm">
                  Rechazas la propuesta de {pagoBet.userId?.substring(0, 8)}... para cubrir tu apuesta
                </div>
              </div>
            </button>
          </div>
