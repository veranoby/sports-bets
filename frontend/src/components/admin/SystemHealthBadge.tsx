...
const SystemHealthBadge: React.FC<SystemHealthBadgeProps> = ({ 
  ...
  if (loading) {
    return (
      <div className={`inline-flex items-center gap-1.5 font-medium text-gray-700 bg-gray-100 border border-gray-300 ${sizeClasses[size]} ${className}`}>
    ...
    return (
      <div className={`inline-flex items-center gap-1.5 font-medium text-red-700 bg-red-100 border border-red-300 ${sizeClasses[size]} ${className}`}>
    ...
    return (
      <Link 
        ...
        title={alertCount > 0 ? `View ${alertCount} system alert${alertCount > 1 ? 's' : ''}` : 'System monitoring - All OK'}
      >
        <div className={`inline-flex items-center gap-1.5 font-medium ${severityColor} ${sizeClasses[size]} ${className}`}>
          {getSeverityIcon()}
          <span>{alertCount > 0 ? `${alertCount} Alert${alertCount > 1 ? 's' : ''}` : 'OK'}</span>
        </div>
      </Link>
    );
  }
