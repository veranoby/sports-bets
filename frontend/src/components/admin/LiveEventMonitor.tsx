...
const LiveEventMonitor: React.FC = () => {
  ...
  const sseState = useMultiSSE<FightStatusData | StreamViewerData | BetData>(channels);
  ...
  useEffect(() => {
    ...
    if (fightEvent && fightEvent.type === 'FIGHT_STATUS_UPDATE') {
      ...
      setFights(prevFights => ({ ...prevFights, [fightData.fightId]: fightData }));
    }
    ...
    if (streamEvent && streamEvent.type === 'VIEWER_COUNT_UPDATE') {
      ...
      setStreams(prevStreams => ({ ...prevStreams, [streamData.eventId]: streamData }));
    }

    const betEvent = sseState.bets?.lastEvent;
    if (betEvent && (betEvent.type === 'NEW_BET' || betEvent.type === 'PAGO_PROPOSAL')) {
  ...
  return (
    <div className="bg-white shadow-lg rounded-lg p-4 border border-gray-200">
      <h3 className="text-lg font-bold text-gray-800 mb-3">Live Event Monitor</h3>
        ...
        <div>
          ...
          <div className="mt-2 space-y-2">
            {Object.values(fights).map(fight => (
              <div key={fight.fightId} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                <span>{fight.title}</span>
                <span className={`px-2 py-1 text-xs text-white rounded-full ${getStatusColor(fight.status)}`}>
                  {fight.status}
                </span>
              </div>
            ))}
             {Object.keys(fights).length === 0 && <p className='text-gray-400 text-sm'>No fight updates yet.</p>}
          </div>
        </div>
          ...
          <h4 className="font-semibold text-gray-700">Streaming</h4>
           <div className="mt-2 space-y-2">
            {Object.values(streams).map(stream => (
              <div key={stream.eventId} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                <span>Event: {stream.eventId}</span>
                <span>Viewers: {stream.viewerCount}</span>
              </div>
            ))}
            {Object.keys(streams).length === 0 && <p className='text-gray-400 text-sm'>No stream updates yet.</p>}
          </div>
        </div>
        ...
        <div>
          ...
          {latestBet ? (
            <div className="p-2 bg-gray-50 rounded">
              <span>{latestBet.type} of ${latestBet.amount}</span>
            </div>
          ) : (
