// frontend/src/components/shared/PaymentForm.tsx
// ✅ UNIFIED PaymentForm - Merged from payments/ and subscriptions/ components
...
// Support both payment contexts
type PaymentContext =
  | { type: "generic"; amount: number; description: string }
  | { type: "subscription"; planType: "daily" | "monthly"; planPrice: number };
...
const PaymentForm: React.FC<PaymentFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Determinar monto y descripción según contexto
      const amount =
        context.type === "generic" ? context.amount : context.planPrice;
      const description =
        context.type === "generic"
          ? context.description
          : `Subscription: ${context.planType === "daily" ? "Daily Access" : "Monthly Premium"}`;
      ...
      const transactionId = await processPayment({
        ...
        currency: "USD",
        description,
        email: "user@example.com", // TODO: Obtener del AuthContext
        token,
      });

      // 4. Callback con resultado según contexto
      if (context.type === "subscription") {
        onSuccess({ token, planType: context.planType });
      } else {
  ...
  const description =
    context.type === "generic"
      ? context.description
      : `${context.planType === "daily" ? "Daily Access" : "Monthly Premium"}`;
